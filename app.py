...
st.sidebar.markdown("---")
with st.sidebar.expander("🛠️ Global System Settings", expanded=False):
    # 1. MT5 Connectivity
    st.subheader("MT5 Connectivity")
    saved_creds = load_credentials()
    is_mt5_connected = st.session_state.get("mt5_connected", False)
    
    if not is_mt5_connected:
        mt5_login = st.number_input("Login", value=saved_creds["login"], step=1, key="global_mt5_login")
        mt5_password = st.text_input("Password", type="password", value=saved_creds["password"], key="global_mt5_pass")
        mt5_server = st.text_input("Server", value=saved_creds["server"], key="global_mt5_server")
        if st.button("🔗 Connect MT5", type="primary", use_container_width=True):
            from mt5_trader import MT5Config, connect_mt5
            new_cfg = MT5Config(
                login=int(mt5_login), password=mt5_password, server=mt5_server,
                symbol=saved_creds.get("symbol", "XAUUSD"),
                max_lot=float(saved_creds.get("max_lot", 0.10))
            )
            ...
            if ok:
                ...
                save_credentials(mt5_login, mt5_password, mt5_server, 
                                 new_cfg.symbol, new_cfg.max_lot)
                st.rerun()
            else:
                st.error(msg)
    else:
        ...
        if acc:
            st.success(f"Connected: {acc['login']}")
            st.markdown(f"💰 Balance: **${acc['balance']:,.2f}**")
            st.markdown(f"📈 Equity: **${acc['equity']:,.2f}**")
            _pnl = acc.get("profit", 0)
            _pnl_color = "green" if _pnl >= 0 else "red"
            st.markdown(f"📊 PnL: :[{_pnl:,.2f}]{{{_pnl_color}}}")
        else:
            st.warning("Connected (retrieving info...)")
            
        if st.button("🔌 Disconnect", use_container_width=True):
            from mt5_trader import disconnect_mt5
            disconnect_mt5()
    ...
    st.markdown("---")
    # 2. Global Guardrails
    st.subheader("Global Guardrails")
    global_max_lot = st.number_input("Max Lot Size", value=float(saved_creds.get("max_lot", 0.10)), step=0.01, key="global_max_lot")
    global_max_spread = st.number_input("Max Spread ($)", value=float(saved_creds.get("max_spread_price", 0.60)), step=0.05, key="global_max_spread")
    global_max_drift = st.number_input("Max Entry Drift ($)", value=float(saved_creds.get("max_entry_drift_price", 2.00)), step=0.10, key="global_max_drift")
    global_max_pos = st.number_input("Max Open Positions", value=int(saved_creds.get("max_open_positions", 2)), step=1, key="global_max_pos")
    global_max_daily_loss_pct = st.slider("Max Daily Loss (%)", min_value=0.1, max_value=10.0, value=float(saved_creds.get("max_daily_loss_pct", 4.0)), step=0.1, key="global_max_daily_loss_pct")
    
    st.markdown("---")
    # 3. Alert Configuration
    st.subheader("Alert Config")
    alert_cfg = load_alert_config()
    alert_enabled = st.toggle("Enable Alerts", value=bool(alert_cfg.get("enabled", False)), key="global_alert_enabled")
    webhook_url = st.text_input("Webhook URL", value=str(alert_cfg.get("webhook_url", "")), key="global_webhook_url")
    if st.button("💾 Save Global Settings", use_container_width=True):
        # Update MT5 config in state
        if is_mt5_connected:
            cfg = st.session_state["mt5_config"]
            cfg.max_lot = global_max_lot
            cfg.max_spread_price = global_max_spread
            cfg.max_entry_drift_price = global_max_drift
            st.session_state["mt5_config"] = cfg
        
        # Save to disk
        save_credentials(
            st.session_state.get("global_mt5_login", saved_creds["login"]),
            st.session_state.get("global_mt5_pass", saved_creds["password"]),
            st.session_state.get("global_mt5_server", saved_creds["server"]),
            saved_creds.get("symbol", "XAUUSD"),
            global_max_lot,
            global_max_spread,
            global_max_drift
        )
        save_alert_config({
            ...
            "notify_on_worker_error": alert_cfg.get("notify_on_worker_error", True),
            "notify_on_worker_failed": alert_cfg.get("notify_on_worker_failed", True),
            "notify_on_watchdog_restart": alert_cfg.get("notify_on_watchdog_restart", True),
            "cooldown_seconds": alert_cfg.get("cooldown_seconds", 120),
        })
...
@st.cache_data(ttl=_cache_ttl, show_spinner="Computing ICT concepts …")
def load_ict_data(sym: str, tf: str) -> dict[str, pd.DataFrame]:
    df = load_data(sym, tf)
    if df.empty:
        return {}
...
def load_signals(sym: str, tf: str, concepts_key: str,
    ...
    if df.empty:
        return pd.DataFrame()
    ict = load_ict_data(sym, tf)
    concepts = concepts_key.split("|") if concepts_key else []
    return generate_signals(df, concepts, rr_ratio=_rr, sweep_lookback=_lookback, ict_data=ict)
...
if page == "Grid Trading":
    ...
    with st.expander("⚙️ Grid Configuration", expanded=not gs.active):
        st.info(f"📍 Operating on **{symbol}** | **{timeframe}** (set in Sidebar Market section)")
        ...
        st.info("💡 Portfolio risk limits (Max Lot, Max DD, Max Positions) are managed in the **Sidebar Global System Settings**.")
    ...
    _grid_cfg = GridConfig(
        ...
        session_pause_list=[],
        max_daily_loss_usd=float(st.session_state.get("global_max_daily_loss_pct", 4.0)) * 100, # Approximation or just use the pct
        max_drawdown_pct=float(st.session_state.get("global_max_drawdown_pct", 10.0)),
        min_equity_usd=0.0,
    )
    # Define portfolio variables from global sidebar settings
    grid_enforce_portfolio_caps = True
    grid_portfolio_max_positions = st.session_state.get("global_max_pos", 2)
    grid_portfolio_max_lots = st.session_state.get("global_max_lot", 0.10)
    grid_account_preset = "Custom"
    ...
    _htf_tf = _HTF_MAP.get(timeframe, "1h")
    ...
    _grid_df, _grid_sigs = _grid_fetch_signals(
        symbol, timeframe, _concepts_default, 20, 1.5
    )
    _htf_df = _grid_fetch_data(symbol, _htf_tf)
    ...
    if _grid_df is not None and not _grid_df.empty:
        ...
        _brain_rec = get_recommendation(symbol, _base_regime.regime, brain)
        ...
        if not gs.active:
            if st.button(
                ...
                with st.spinner("Activating grid, please wait…"):
                    try:
                        _brain_mult2 = (
                            get_recommendation(symbol, _regime.regime, brain)["spacing_multiplier"]
                        ...
                        _start_balance = 0.0
                        if _mt5_ok:
                            _tick = get_live_price(MT5_SYMBOL_MAP.get(symbol, symbol))
                            if _tick:
...
# ===================================================================
    ...
    if not is_connected:
        st.info("👈 Please connect to MT5 using the **Global System Settings** in the sidebar.")
        st.stop()

    # account info already shown at the top of the block if connected
    acc = get_account_info()
    if acc:
        _pnl = acc.get("profit", 0)
    ...
    st.info("⚙️ Execution settings are managed globally in the **Sidebar > Global System Settings**.")

    # Use stored config
    config = st.session_state.get("mt5_config", MT5Config())
    # Override with global sidebar values
    config.max_lot = st.session_state.get("global_max_lot", config.max_lot)
    config.max_spread_price = st.session_state.get("global_max_spread", config.max_spread_price)
    config.max_entry_drift_price = st.session_state.get("global_max_drift", config.max_entry_drift_price)
    config.kill_switch = st.session_state.get("mt5_kill_switch", config.kill_switch)
    st.session_state["mt5_config"] = config

    # ── Quick Policy Overview ───────────────────────────
    st.markdown("---")
    st.subheader("Current Policy Status")
    cs1, cs2, cs3, cs4 = st.columns(4)
    cs1.metric("Max Lot", f"{config.max_lot:.2f}")
    cs2.metric("Max Spread", f"${config.max_spread_price:.2f}")
    cs3.metric("Max Drift", f"${config.max_entry_drift_price:.2f}")
    cs4.metric("Trades/Day", str(config.max_trades_per_day))

    # Keep Profile & Policy for fine-tuning
    ...
    selected_profile = pp1.selectbox(
        ...
        key="mt5_policy_profile_selector"
    )
