    ...
    for i in range(80, len(work)):
        ...
        for lv in state.levels:
            ...
            if lo <= lv.price <= hi:
                ...
                lv.entry_time = time # track entry time for swap

        # close OPEN levels
        for lv in [x for x in state.levels if x.status == "OPEN"]:
            exit_price = None
            
            # Better intra-bar logic:
            # If both SL and TP are hit in the same bar, we assume the one closer to Open was hit first.
            if lv.direction == "BUY":
                sl_hit = lo <= lv.sl_price
                tp_hit = hi >= lv.tp_price
                if sl_hit and tp_hit:
                    # If SL is closer to Open than TP, hit SL first (conservative)
            ...
            if exit_price is not None:
                ...
                # Costs: Spread + Slippage + Commission + Swap
        ...
        if close_basket:
            for lv in [x for x in state.levels if x.status == "OPEN"]:
                ...
                if lv.direction == "SELL": gross = -gross
